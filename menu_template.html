<!DOCTYPE html>
<html>
<head>
	<title>Menu template</title>
	<link href='https://fonts.googleapis.com/css?family=Open+Sans' rel='stylesheet' type='text/css'>
	<link rel="stylesheet" type="text/css" href="./css/style.css">
</head>
<body>
	<div class="wrapper">
	    <header>
	    	<div class="topbar">
		    	<div class="logo">
		    		<img id="logo" src="./images/logo.png">
		    	</div>
		    	<!-- username -->
		    	<div class="top_username">
		    		<span>Username&nbsp;<img id="dropdown_icon" src="./images/dropdown.png"></span>
		    	</div>
	    	</div>
	    	<!-- menu -->
	    	<nav class="secondbar">
	    		<ul class="top_menu">
					<li class="menu_active"><a href="#" class="secondbar_item">Rentals</a>
					    <ul class="dropdown">
					    	<li><a href="#">test</a></li>
					    	<li><a href="#">test</a></li>
					    	<li><a href="#">test</a></li>
					    	<li><a href="#">test</a></li>
					    </ul>
					</li>
					<li><a href="#" class="secondbar_item">Returns</a></li>
					<li><a href="#" class="secondbar_item">File Maintenance</a></li>
					<li><a href="#" class="secondbar_item">Customers</a></li>
					<li><a href="#" class="secondbar_item">Reports</a></li>
	    		</ul>
	    	</nav>
	    </header>
	    <!-- content -->
	    <section class="content">
	    	<div class="container">
		    	<!-- form -->
		    	<div class="form"></div>
		    	<!-- image -->
		    	<div class="home_car">
		    		<img id="car" src="./images/car.png">
		    	</div>
	    	</div>
	    </section>

	    <footer>
	    	<span class="copyright">Copyright &copy; 2016 All rights reserved.</span>
	    </footer>
    </div>
</body>
</html>